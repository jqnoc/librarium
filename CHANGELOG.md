...
- Versioning paradigm updated: `APP_VERSION` and `package.json` are now bumped to `x.y.z-beta.1` as soon as the Unreleased changelog section contains any entry (minor bump for `Added`/`Changed`, patch bump for `Fixed`/`Removed` only); the beta suffix is stripped when a release is cut
