...
- "Generate Infographic" button at the bottom of the Books Finished in YYYY page exports a PNG image with a dark-themed layout showing all book covers in an auto-fitted grid, the year, book count badge, per-book star rating pill on each cover (gold star + score, semi-transparent dark pill with gold border), the Librarium version in the footer, and an accent separator; width and height are selectable in a modal before exporting
