...
- "Generate Infographic" button at the bottom of the Books Finished in YYYY page exports a PNG image with a dark-themed layout showing all book covers in an auto-fitted grid, the year, stat badges (book count, pages read, hours read), per-book star rating pill on each cover (deep-orange star + score), the Librarium version in the footer, and an accent separator; width and height are selectable in a modal before exporting; modal is appended to `document.body` to avoid stacking-context issues
