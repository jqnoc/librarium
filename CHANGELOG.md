...
- Add timed Reading-card sessions for page-based readings with book identity, centered live page ranges using page deltas, Space-bar pause/resume control, pause-to-edit spinboxes that preserve manual adjustments when saved, selected-reading attachment, X-only closing, a content-sized responsive dialog with padded no-wrap annotation buttons and a full-width lower action row preserving the rendered Save Session minimum, Ctrl + Enter bullet insertion, Ctrl + ç `#### ` heading insertion, and `### ` heading initialization for new Thoughts, centered playback controls beneath the annotation row, and paused Classification/Add Quote/Add Word/Add Thought dialogs that close only through their X buttons or successful saves, with public dictionary lookup, translation, latest-thought editing, and New Thought flow
