...
- "Generate Infographic" button at the bottom of the Books Finished in YYYY page exports a PNG image with a dark-themed layout showing all book covers in an auto-fitted grid, the year, stat badges (book count, pages read, hours read), per-book rating pill styled like the Library card-view badge with a `#e3bc37` star fill, yellow border, and blue score text, the Librarium version in the footer, and an accent separator; width and height are selectable in a properly rendered modal that reuses the app's modal styling and is appended to `document.body` before exporting
