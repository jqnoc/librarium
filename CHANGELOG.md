...
### Added
- Add a reusable session-based back button with bounded history across page visits, including POST redirects and refreshes
...
- Add a Start Session action beside the active Reading card and show Resume Last Session there with a distinct continuation icon when the last session is from today, reusing its saved page range, date, and elapsed time and removing the redundant session-row action
- Add Markdown downloads for each book's classification and character data without portraits
- Add a read-only `Wind and Truth` classification analysis comparing the target taxonomy with existing classified books and recommending normalization and cross-book review
- Add a read-only JqnOC taxonomy duplicate audit covering all 723 books, effective work-level classifications, semantic duplicate candidates, affected books, and false-positive guidance
- Add timed Reading-card sessions for page-based readings with book identity, centered live page ranges using page deltas, Space-bar pause/resume control, pause-to-edit spinboxes that preserve manual adjustments when saved, selected-reading attachment, X-only closing, a content-sized responsive dialog with a concise four-action horizontal annotation toolbar and a full-width lower action row preserving the rendered Save Session minimum, Ctrl + Enter bullet insertion, Ctrl + ç `#### ` heading insertion, and `### ` heading initialization for new annotations, centered playback controls beneath the annotation row, paused Classification/Add Quote/Add Word/Add Annotation dialogs that close only through their X buttons or successful saves, a rich-text toolbar for session quotes, book-scoped persistence for Dictionary Source, Dictionary Language, and Translation Language selections, public dictionary lookup, translation, latest-annotation editing, and New Annotation flow
- Add persisted per-book Daily or Weekly finish plans using one expected finish date, with inclusive daily or calendar-week page and time targets based on the selected reading's progress and reading pace, including current-period progress, actual time read, and goal-reached states
...
- Show per-session pace as `pages/hour` with signed average differences and centered comparison bars in a separate titleless column between Pace and Actions, coloring bars from slowest red through fastest green based on the visible session or day range
...
- Book word annotations now support an optional translated term plus a translation-language selector, remember the last selected translation language in the word editor, and show translations on the book detail page and dashboard word spotlights
- Annotation editor now uses Markdown instead of raw HTML; a dedicated Markdown toolbar provides formatting buttons for bold, italic, strikethrough, headings, lists, blockquotes, links, code, and horizontal rules
...
### Fixed
- Quote of the Day on the Dashboard and book-detail quotes now render curly quotes correctly instead of showing literal `\u201c` / `\u201d` escape sequences; also fixes HTML formatting within quotes being escaped
...
### Added
- Annotations system: quotes, annotations, and words per book edition with dedicated database tables, full CRUD (add, edit, delete) inline on the book detail page; quotes and annotations ordered by page number (null-page entries last), words ordered alphabetically in a responsive grid
- Bookly PDF import on the edit metadata page: upload a Bookly summary PDF to automatically extract and import quotes, annotations, and words; optional checkbox to clear existing annotations before importing; quotation marks are automatically stripped from extracted quotes
...
- Rich-text editing toolbar (bold, italic, underline, strikethrough, headings, lists, links) for quote, annotation, and word textareas on the book detail page
...
### Changed
- Annotations, Quotes, and Words sections on the book detail page moved from after Reading Periods to between Reading Stats and Reading Sessions
