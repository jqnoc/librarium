...
- Add New Book and Edit Metadata now share the same ordered metadata form layout, including full-width Classification chip editing for alphabetically sorted Genres, Subgenres, Themes, Settings, Historical Periods, Subjects, and Audiences with descriptions between each category label and its editor, semicolon-delimited values, and comma-containing value support, while preserving their mode-specific controls; Publication Details use the requested three-column rows, Original Work uses one three-column row, Contributors use two three-column rows with Foreword/Introduction, Epilogue, and Contributing Author fields, and Book Source fields use three-column rows while preserving existing Prologue Author data; paper-book bindings include Softcover
