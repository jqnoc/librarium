...
- Add book character annotations with Name, Description, Biography, optional 4:5 Portraits, required Importance, multi-select Role fields, inline unlabeled translated colored badges, summary cards with right-aligned portrait thumbnails and biography detail modals with blurred backgrounds, consistent annotation form styling, Book Details CRUD controls, manual drag-and-drop ordering, Reading Session creation/editing from a character selector inside the dialog, and position preservation after inline edits
